...
class ProductAdapter(val context: Context, var datalist: ArrayList<ProductModal>) :
    RecyclerView.Adapter<ProductAdapter.MyViewHolder>() {


    private val sharedPref = context.getSharedPreferences("userdetails", Context.MODE_PRIVATE)
    private val uid = sharedPref.getString("userid", "") ?: ""
    private val databaseReference: DatabaseReference = FirebaseDatabase.getInstance().getReference("users").child(uid)
    ...
    override fun onBindViewHolder(holder: MyViewHolder, position: Int) {
       ...
       addproduct(datalist.get(position), holder)
        }

        val a = datalist.get(position).productid.toString()
        syncCartState(a, holder)
    ...
    fun syncCartState(a: String, holder: MyViewHolder) {
        databaseReference.addValueEventListener(object : ValueEventListener {
            override fun onDataChange(snapshot: DataSnapshot) {
                ...
                holder.decIV.setOnClickListener {
                    if (number!! > 1) {
                        number = number!! - 1
                        databaseReference.child("cart").child(a).child("productnumber")
                            .setValue(number)
                    } else {
    ...
    private fun addproduct(product: ProductModal, holder: MyViewHolder) {
        val a = product
        ...
        databaseReference.child("cart").child(product.productid.toString()).setValue(productData)
